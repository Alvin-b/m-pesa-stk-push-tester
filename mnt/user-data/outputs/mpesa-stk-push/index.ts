...
serve(async (req) => {
  ...
  try {
    ...
    // FIX: Create voucher as 'pending' — not 'active' — until payment is confirmed.
    // The portal polls daraja-stk-query and on success looks up the voucher by
    // checkout_request_id. We activate it and create RADIUS credentials only here
    // so that only paid vouchers get internet access.
    if (packageId) {
      ...
      // Insert as 'pending' — activated by confirm-payment after polling succeeds
      await sb.from('vouchers').insert({
        code,
        package_id: packageId,
        phone_number: formattedPhone,
        checkout_request_id: stkData.CheckoutRequestID,
        status: 'pending',          // FIX: was 'active'
        session_timeout: sessionTimeout,
      });

      // FIX: Do NOT create radcheck/radreply here yet.
      // They are created in confirm-payment after payment is verified,
      // so only paid users get RADIUS credentials.
    }

...
const Admin = () => {
  ...
  // New package form
  const [newPkg, setNewPkg] = useState({ name: "", description: "", duration_minutes: 60, price: 20, speed_limit: "5M/5M" });
  ...
  const addPackage = async () => {
    ...
    setNewPkg({ name: "", description: "", duration_minutes: 60, price: 20, speed_limit: "5M/5M" });
    setSavingPkg(false);
    loadData();
  };
...
# Upload this file to your MikroTik and run: /import file-name=hotspot-setup.rsc
...
# 4. Setup RADIUS Client
# Configure this to point to your FreeRADIUS server
/radius
...
# 5. Set login page redirect to billing portal
# Upload the hotspot HTML files that redirect to: ${portalUrl}
              ...
              <Card>
                ...
                <CardContent>
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                    <Input placeholder="Name" value={newPkg.name} onChange={(e) => setNewPkg({ ...newPkg, name: e.target.value })} className="font-mono bg-muted/50" />
                    <Input placeholder="Description" value={newPkg.description} onChange={(e) => setNewPkg({ ...newPkg, description: e.target.value })} className="font-mono bg-muted/50" />
                    <Input type="number" placeholder="Minutes" value={newPkg.duration_minutes} onChange={(e) => setNewPkg({ ...newPkg, duration_minutes: parseInt(e.target.value) || 0 })} className="font-mono bg-muted/50" />
                    <Input type="number" placeholder="Price (KES)" value={newPkg.price} onChange={(e) => setNewPkg({ ...newPkg, price: parseFloat(e.target.value) || 0 })} className="font-mono bg-muted/50" />
                    ...
                    <TableBody>
                      {packages.filter(p => p.is_active).map((p) => (
                        <TableRow key={p.id}>
                          ...
                          <TableCell className="font-mono">{p.duration_minutes}min</TableCell>
